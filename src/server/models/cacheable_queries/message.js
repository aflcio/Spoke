...
const incomingMessageMatching = async (messageInstance, activeCellFound) => {
  if (!activeCellFound) {
    ...
    log.info({
      category: 'messageCache',
      event: 'ORPHAN MESSAGE',
      messageInstance,
      activeCellFound
    });
...
const deliveryReport = async ({
  ...
  if (newStatus === "ERROR") {
    ...
    log.info({category: 'messageCache', event: 'deliveryReport', lookup});
